...
class BlacklistCommand extends Command {
  ...
  async execute({ ctx }) {
    ...
    switch (action) {
      ...
      default:
        return ctx.reply(
          `**Invalid action!** Use: add, remove, check, or list`,
        );
    }
  }
  async handleAdd(ctx) {
    ...
    if (await db.blacklist.checkBlacklist(id))
      return ctx.reply("Already blacklisted");

    try {
      if (type === "user")
        await db.blacklist.blacklistUser(id, ctx.user.id, reason);
  ...
  async handleRemove(ctx) {
    ...
    try {
      ...
      return ctx.reply(
        `**Removed!** ID \`${id}\` has been removed from the blacklist.`,
      );
    } catch (error) {
  ...
  async handleCheck(ctx) {
    ...
    if (!entry) {
      return ctx.reply(
        ` **Not Blacklisted!** ID \`${id}\` is not in the blacklist.`,
      );
    }
  ...
  async handleList(ctx) {
    ...
    if (!entries || entries.length === 0) {
      return ctx.reply(
        `**No Entries!** No ${type || "blacklisted"} entries found.`,
      );
    }
