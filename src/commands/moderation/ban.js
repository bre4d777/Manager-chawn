...
class BanCommand extends Command {
	constructor() {
		super({
			...
			examples: ['ban @user', 'ban @user 7 spamming', 'ban 123456789 1 rule violation'],
			cooldown: 5,
	...
	async execute({ ctx }) {
		if (!ctx.inGuild()) {
			return ctx.reply({
				components: [this._errorView('This command can only be used in a server.')],
		...
		if (!botMember.permissions.has(PermissionFlagsBits.BanMembers)) {
			return ctx.reply({
				components: [this._errorView('I do not have permission to ban members.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!ctx.member.permissions.has(PermissionFlagsBits.BanMembers)) {
			return ctx.reply({
				components: [this._errorView('You do not have permission to ban members.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (ctx.isSlash) {
			...
			if (!rawTarget) {
				return ctx.reply({
					components: [this._errorView('Please provide a user to ban.\n\n**Usage:** `/mod ban <user> [days] [reason]`')],
			...
			try {
				...
				return ctx.reply({
					components: [this._errorView(`Could not find a user with ID or mention \`${rawTarget}\`.`)],
					flags: MessageFlags.IsComponentsV2,
				});
			}
			...
			if (rawDays && isNaN(parsedDays)) {
				...
				deleteMessageDays = Math.min(Math.max(parsedDays || DEFAULT_DELETE_DAYS, 0), MAX_DELETE_DAYS);
			}
		...
		if (targetMember) {
			if (!targetMember.bannable) {
				return ctx.reply({
					components: [this._errorView(`I cannot ban **${target.tag}** — their role is higher than or equal to mine.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
			...
			if (executorHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
				return ctx.reply({
					components: [this._errorView(`You cannot ban **${target.tag}** — their role is higher than or equal to yours.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
		}
		...
		if (alreadyBanned) {
			return ctx.reply({
				components: [this._errorView(`**${target.tag}** is already banned from this server.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		try {
			...
			return ctx.reply({
				components: [this._errorView(`Failed to ban **${target.tag}**: ${err.message}`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}

		await ctx.reply({
			components: [this._successView(target, ctx.user, reason, deleteMessageDays)],
			flags: MessageFlags.IsComponentsV2,
		});
	}
