...
class HackbanCommand extends Command {
	...
	async execute({ ctx }) {
		...
		if (ctx.isSlash) {
			...
			if (!id) {
				return ctx.reply({
					components: [_errorView('Please provide a user ID.\n\n**Usage:** `hackban <userId> [days] [reason]`')],
			...
			if (rawDays && isNaN(parsedDays)) {
				...
				deleteMessageDays = Math.min(Math.max(parsedDays || 0, 0), MAX_DELETE_DAYS);
			}
		...
		if (!/^\d{17,20}$/.test(rawId)) {
			return ctx.reply({
				components: [_errorView(`\`${rawId}\` is not a valid Discord user ID. IDs are 17–20 digits.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (existingBan) {
			return ctx.reply({
				components: [_errorView(`User \`${rawId}\` is already banned from this server.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (targetMember) {
			if (!targetMember.bannable) {
				return ctx.reply({
					components: [_errorView(`I cannot ban <@${rawId}> — their role is too high.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
			...
			if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
				return ctx.reply({
					components: [_errorView(`You cannot ban <@${rawId}> — their role is higher than or equal to yours.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
		}
		...
		return ctx.reply({
			components: [_successView(resolvedUser, rawId, ctx.user, reason, deleteMessageDays)],
			flags: MessageFlags.IsComponentsV2,
		});
	}
}
