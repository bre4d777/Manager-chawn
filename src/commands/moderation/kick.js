...
class KickCommand extends Command {
	...
	async execute({ ctx }) {
		...
		if (!targetMember.kickable) {
			return ctx.reply({
				components: [_errorView(`I cannot kick **${target.tag}** — their role is too high.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
			return ctx.reply({
				components: [_errorView(`You cannot kick **${target.tag}** — their role is higher than or equal to yours.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		try {
			...
			return ctx.reply({
				components: [_errorView(`Failed to kick **${target.tag}**: ${err.message}`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
