...
class NicknameCommand extends Command {
  constructor() {
    super({
      ...
      slashData: {
        ...
        options: [
          { name: 'user',     description: 'Member to change nickname for',      type: 6, required: true  },
  ...
  async execute({ ctx }) {
    ...
    if (newNick && newNick.length > MAX_NICK_LENGTH) {
      return ctx.reply({
        components: [_errorView(`Nickname must be ${MAX_NICK_LENGTH} characters or fewer (got ${newNick.length}).`)],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }
    ...
    if (target.id !== ctx.user.id) {
      ...
      if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
        return ctx.reply({
          components: [_errorView(`You cannot change **${target.tag}**'s nickname — their role is higher than or equal to yours.`)],
          flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        });
      }
    }
    ...
    try {
      ...
      return ctx.reply({
        components: [_errorView(`Failed to change nickname for **${target.tag}**: ${err.message}`)],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }
...
function _successView(target, executor, oldNick, newNick, isReset) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(isReset ? '## Nickname Reset' : '## Nickname Changed'),
  );
