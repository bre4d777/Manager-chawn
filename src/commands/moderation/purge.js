...
class PurgeCommand extends Command {
	constructor() {
		super({
			...
			usage: 'purge <amount> [--user @u] [--bots] [--humans] [--contains text] [--startswith text] [--embeds] [--attachments] [--mentions] [--links] [--regex pattern] [--reason text]',
			examples: [
				'purge 100',
				'purge 50 --user @user',
				'purge 200 --bots',
			...
			slashData: {
				...
				options: [
					...
					{ name: 'user',        description: 'Only messages from this user',         type: 6, required: false },
					{ name: 'bots',        description: 'Only bot messages',                    type: 5, required: false },
					{ name: 'humans',      description: 'Only human messages',                  type: 5, required: false },
					{ name: 'contains',    description: 'Messages containing this text',        type: 3, required: false },
					{ name: 'startswith',  description: 'Messages starting with this text',     type: 3, required: false },
					{ name: 'embeds',      description: 'Only messages with embeds',            type: 5, required: false },
					{ name: 'attachments', description: 'Only messages with attachments',       type: 5, required: false },
					{ name: 'mentions',    description: 'Only messages mentioning users/roles', type: 5, required: false },
					{ name: 'links',       description: 'Only messages containing links',       type: 5, required: false },
					{ name: 'regex',       description: 'Messages matching this regex pattern', type: 3, required: false },
					{ name: 'reason',      description: 'Reason for the purge',                 type: 3, required: false },
				],
			},
		});
	}
	...
	async execute({ ctx }) {
		...
		if (!botMember.permissions.has(PermissionFlagsBits.ManageMessages)) {
			return ctx.reply({
				components: [_errorView('I do not have permission to manage messages.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}

		if (!botMember.permissionsIn(ctx.channel).has(PermissionFlagsBits.ManageMessages)) {
			return ctx.reply({
				components: [_errorView('I do not have permission to manage messages in this channel.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!ctx.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
			return ctx.reply({
				components: [_errorView('You do not have permission to manage messages.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (ctx.isSlash) {
			...
			if (parsed.rawUser) {
				...
				if (!resolved) {
					return ctx.reply({
						components: [_errorView(`Could not find user \`${parsed.rawUser}\`.`)],
						flags: MessageFlags.IsComponentsV2,
					});
				}
		...
		if (filters.regex) {
			try {
				...
				return ctx.reply({
					components: [_errorView(`Invalid regex pattern: \`${filters.regex}\``)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
		}
		...
		return ctx.editReply({
			components: [_successView(result, filters, ctx.user, reason, ctx.channel)],
			flags: MessageFlags.IsComponentsV2,
		});
	}
}
...
function _matchesFilters(msg, f) {
	...
	if (f.mentions    && msg.mentions.users.size === 0 && msg.mentions.roles.size === 0) return false;
	if (f.links       && !/https?:\/\//i.test(msg.content)) return false;
	if (f.contains    && !msg.content.toLowerCase().includes(f.contains.toLowerCase())) return false;
	if (f.startswith  && !msg.content.toLowerCase().startsWith(f.startswith.toLowerCase())) return false;
	if (f.regex) {
		try { if (!new RegExp(f.regex, 'i').test(msg.content)) return false; }
		catch { return false; }
	}
	return true;
}
...
function _parsePrefixArgs(args) {
	...
	if (isNaN(amount) || amount < 1) return { error: `\`${args[0]}\` is not a valid number.` };
	if (amount > MAX_SCAN)           return { error: `Maximum scan amount is **${MAX_SCAN}**.` };

	const filters = {
		userId: null, bots: false, humans: false, contains: null,
		startswith: null, embeds: false, attachments: false,
		mentions: false, links: false, regex: null,
	};
	...
	while (i < args.length) {
		const tok = args[i];
		if      (tok === '--bots')        { filters.bots        = true;              i++; }
		else if (tok === '--humans')      { filters.humans      = true;              i++; }
		else if (tok === '--embeds')      { filters.embeds      = true;              i++; }
		else if (tok === '--attachments') { filters.attachments = true;              i++; }
		else if (tok === '--mentions')    { filters.mentions    = true;              i++; }
		else if (tok === '--links')       { filters.links       = true;              i++; }
		else if (tok === '--contains')    { filters.contains    = args[++i] ?? null; i++; }
		else if (tok === '--startswith')  { filters.startswith  = args[++i] ?? null; i++; }
		else if (tok === '--regex')       { filters.regex       = args[++i] ?? null; i++; }
		else if (tok === '--user')        { rawUser             = args[++i] ?? null; i++; }
		else if (tok === '--reason')      { reasonParts.push(...args.slice(i + 1));  i = args.length; }
		else                              { i++; }
	}
