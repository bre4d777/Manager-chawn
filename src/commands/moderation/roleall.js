...
class RoleAllCommand extends Command {
  ...
  async execute({ ctx }) {
    ...
    return ctx.editReply({
      components: [_finalView(role, action, result, ctx.user, reason, "All members")],
      flags: MessageFlags.IsComponentsV2,
    });
  }
}
...
async function _runMassRole(ctx, role, action, baseFilter, excludeRoleId, includeRoleId, reason) {
  ...
  const auditReason = _buildAuditReason(ctx.user, action === "add" ? "RoleAll-Add" : "RoleAll-Remove", reason);
