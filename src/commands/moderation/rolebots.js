...
class RoleBotsCommand extends Command {
  constructor() {
    super({
      ...
      usage:
        "rolebots <add|remove> <@role> [--exclude @role] [--include @role] [--reason text]",
      examples: [
...
if (parsed.excludeRoleId && !excludeRole) {
    ...
    return ctx.editReply({
      components: [
        _finalView(role, action, result, ctx.user, reason, "All bots"),
      ],
      flags: MessageFlags.IsComponentsV2,
    });
  }
...
async function _runMassRole(
  ...
  const auditReason = _buildAuditReason(
    ctx.user,
    action === "add" ? "RoleBots-Add" : "RoleBots-Remove",
    reason,
  );
