...
class RoleBotsCommand extends Command {
  constructor() {
    super({
      name: "rolebots",
      description: "Add or remove a role from every bot in the server",
      usage: "rolebots <add|remove> <@role> [--exclude @role] [--include @role] [--reason text]",
      examples: [
        "rolebots add @BotRole",
        "rolebots remove @BotMuted",
        "rolebots add @Bots",
        "rolebots remove @BotEvent",
      ],
      aliases: ["massbots", "bulkbots"],
      cooldown: 60,
  ...
  async execute({ ctx }) {
    ...
    return ctx.editReply({
      components: [_finalView(role, action, result, ctx.user, reason, "All bots")],
      flags: MessageFlags.IsComponentsV2,
    });
  }
}
...
async function _runMassRole(ctx, role, action, baseFilter, excludeRoleId, includeRoleId, reason) {
  ...
  const auditReason = _buildAuditReason(ctx.user, action === "add" ? "RoleBots-Add" : "RoleBots-Remove", reason);
...
export default new RoleBotsCommand();