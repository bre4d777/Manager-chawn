...
class RoleRemoveCommand extends Command {
  constructor() {
    super({
      ...
      usage: "roleremove <user> <role> [reason]",
      examples: [
        "roleremove @user @Member",
        "roleremove @user @Muted timeout expired",
        "roleremove 123456789 @Verified",
      ],
      aliases: ["removerole", "takerole", "delrole"],
      cooldown: 5,
  ...
  async execute({ ctx }) {
    ...
    if (role.id === ctx.guild.roles.everyone.id) {
      return ctx.reply({
        components: [_errorView("The @everyone role cannot be removed.")],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }
    ...
    if (role.position >= botMember.roles.highest.position) {
      return ctx.reply({
        components: [
          _errorView(
            `I cannot remove ${role} — it is higher than or equal to my highest role.`,
          ),
        ],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    ...
    if (role.position >= execHighest && ctx.guild.ownerId !== ctx.user.id) {
      return ctx.reply({
        components: [
          _errorView(
            `You cannot remove ${role} — it is higher than or equal to your highest role.`,
          ),
        ],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    ...
    if (!targetMember.roles.cache.has(role.id)) {
      return ctx.reply({
        components: [_errorView(`**${target.tag}** does not have ${role}.`)],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }

    const auditReason = _buildAuditReason(ctx.user, "RoleRemove", reason);
...
function _successView(target, role, executor, reason) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## Role Removed"),
  );
...
export default new RoleRemoveCommand();
