...
class TimeoutCommand extends Command {
	constructor() {
		super({
			...
			examples: ['timeout @user 10m', 'timeout @user 2h spam', 'timeout @user 1d raid'],
	...
	async execute({ ctx }) {
		...
		if (!botMember.permissions.has(PermissionFlagsBits.ModerateMembers)) {
			return ctx.reply({
				components: [_errorView('I do not have permission to timeout members.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!ctx.member.permissions.has(PermissionFlagsBits.ModerateMembers)) {
			return ctx.reply({
				components: [_errorView('You do not have permission to timeout members.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (ctx.isSlash) {
			...
			if (!rawDur) {
				return ctx.reply({
					components: [_errorView('Please provide a duration (e.g. `10m`, `2h`, `1d`, `1w`).')],
					flags: MessageFlags.IsComponentsV2,
				});
			}
		...
		if (!durationMs) {
			return ctx.reply({
				components: [_errorView('Invalid duration. Use formats like `10m`, `2h`, `1d`, `1w`. Maximum is 28 days.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!targetMember.moderatable) {
			return ctx.reply({
				components: [_errorView(`I cannot timeout **${target.tag}** — their role is too high.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
			return ctx.reply({
				components: [_errorView(`You cannot timeout **${target.tag}** — their role is higher than or equal to yours.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		try {
			...
			return ctx.reply({
				components: [_errorView(`Failed to timeout **${target.tag}**: ${err.message}`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
