...
class UnbanCommand extends Command {
	...
	async execute({ ctx }) {
		...
		if (!ctx.member.permissions.has(PermissionFlagsBits.BanMembers)) {
			return ctx.reply({
				components: [_errorView('You do not have permission to unban members.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!/^\d{17,20}$/.test(rawId)) {
			return ctx.reply({
				components: [_errorView(`\`${rawId}\` is not a valid Discord user ID.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!banEntry) {
			return ctx.reply({
				components: [_errorView(`No active ban found for user ID \`${rawId}\`.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		try {
			...
			return ctx.reply({
				components: [_errorView(`Failed to unban \`${rawId}\`: ${err.message}`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}

		return ctx.reply({
			components: [_successView(banEntry.user, ctx.user, reason, banEntry.reason)],
			flags: MessageFlags.IsComponentsV2,
		});
	}
}
