...
class UnlockCommand extends Command {
  constructor() {
    super({
      name: "unlock",
      description: "Restore @everyone send permission in a channel",
  ...
  async execute({ ctx }) {
    ...
    if (!existing?.deny.has(PermissionFlagsBits.SendMessages)) {
      return ctx.reply({
        components: [_errorView(`${target} is not locked.`)],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }

    const auditReason = _buildAuditReason(ctx.user, "Unlock", reason);
    ...
    try {
      ...
      return ctx.reply({
        components: [_errorView(`Failed to unlock ${target}: ${err.message}`)],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }
...
function _errorView(description) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(`## Unlock Failed\n\n${description}`),
  );
  return container;
}
...
export default new UnlockCommand();