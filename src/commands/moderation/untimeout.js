...
class UntimeoutCommand extends Command {
	...
	async execute({ ctx }) {
		...
		if (!botMember.permissions.has(PermissionFlagsBits.ModerateMembers)) {
			return ctx.reply({
				components: [_errorView('I do not have permission to manage timeouts.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!ctx.member.permissions.has(PermissionFlagsBits.ModerateMembers)) {
			return ctx.reply({
				components: [_errorView('You do not have permission to manage timeouts.')],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (
			...
			return ctx.reply({
				components: [_errorView(`**${target.tag}** is not currently timed out.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (!targetMember.moderatable) {
			return ctx.reply({
				components: [_errorView(`I cannot modify **${target.tag}**'s timeout — their role is too high.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
			return ctx.reply({
				components: [_errorView(`You cannot modify **${target.tag}**'s timeout — their role is higher than or equal to yours.`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
		...
		try {
			...
			return ctx.reply({
				components: [_errorView(`Failed to remove timeout from **${target.tag}**: ${err.message}`)],
				flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
			});
		}
...
function _errorView(description) {
	...
	container.addTextDisplayComponents(
		new TextDisplayBuilder().setContent(`## Untimeout Failed\n\n${description}`),
	);
	return container;
}
