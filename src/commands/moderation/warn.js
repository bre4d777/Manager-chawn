  ...
  async execute({ ctx }) {
    ...
    if (targetMember) {
      ...
      if (execHighest <= targetHighest && ctx.guild.ownerId !== ctx.user.id) {
        return ctx.reply({
          components: [_errorView(`You cannot warn **${target.tag}** — their role is higher than or equal to yours.`)],
          flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        });
      }
    }
    ...
    const warn      = await db.warns.addWarn(ctx.guild.id, target.id, ctx.user.id, reason);
    ...
    if (threshold && targetMember) {
      try {
        punishment = await db.warns.executePunishment(ctx.guild, targetMember, threshold, reason);
      } catch {
    ...
    return ctx.reply({
      components: [_successView(target, ctx.user, reason, warn.id, warnCount, punishment)],
      flags: MessageFlags.IsComponentsV2,
    });
  }
}
...
function _successView(target, executor, reason, warnId, warnCount, punishment) {
  ...
  if (punishment && !punishment.failed) {
    const actionLabel = punishment.action === 'timeout'
      ? `Timed out for ${punishment.detail}`
      : punishment.action === 'kick' ? 'Kicked' : 'Banned';
    lines.push(`**Threshold Action:** ${actionLabel}`);
  } else if (punishment?.failed) {
    lines.push(`**Threshold Action:** Failed to execute (${punishment.action})`);
  }
