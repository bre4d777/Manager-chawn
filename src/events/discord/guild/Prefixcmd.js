...
const handleMentionOnly = async (message, client, guildPrefixes) => {
  if (
    ...
    !client.user ||
    !guildPrefixes ||
    guildPrefixes.length === 0
  )
    return false;
  try {
    const mentionRegex = getMentionRegex(client.user.id);
    if (!mentionRegex || !mentionRegex.test(message.content.trim()))
      return false;
...
const getCommand = (parts, commandHandler) => {
  if (!parts || parts.length === 0 || !commandHandler)
...
export default {
  ...
  async execute({ eventArgs, client }) {
    ...
    try {
      ...
      try {
        [isUserBlacklisted, isGuildBlacklisted, guildPrefixes] =
          await Promise.all([
