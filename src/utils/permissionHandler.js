...
export const getMissingBotPermissions = (channel, permissions) => {
  if (
    ...
    return (
      permissions?.map((p) => permissionNames.get(p) || "Unknown Permission") ||
      []
    );
  }
  ...
  try {
    ...
    if (!botPerms) {
      return permissions.map(
        (p) => permissionNames.get(p) || "Unknown Permission",
      );
    }
...
export const validateCommand = async (ctx, command) => {
  ...
  if (command.voiceRequired && member.voice?.channel) {
    const voiceMissing = getVoiceChannelMissingPermissions(
